...
float euclideanDist(float* train, float* test){
    float sum = 0.f;
    for (int d=0; d<M; ++d) {
...
int localKnn(float* trainData, float* localTestData, uint8_t* trainClass, uint8_t* localTestClass, int NrecordTesting, int * confusionMatrix){       //distanza da ogni punto del training (dopo ordinamento i primi k saranno i vicini)
        ...
        for(int i = 0; i < NrecordTesting; i++){
            ...
            for(int j = 0; j < N; j++){
                k_distances[j] = euclideanDist(&trainData[j * A], &localTestData[i * A]);
                k_labels[j] = j;
            }
...
void knn( 
    ...
    splitData(countsRow, countsClasses, displsRow, displsClasses, size, N, M); 
    ...
    if(rank ==0){
        ...
        //printData(trainData, trainClass, N);
        //printData(testData, testClass, P);
        ...
        int localError = 0;
        localError = localKnn(trainData, localtestData, trainClass, localtestClass, countsRow[rank] / A, confusionMatrixLocal);
